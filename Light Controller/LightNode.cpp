...
LightNode::LightNode(const std::string& _name,
	...
	infoRequestMsg = Packet::Init().asDatagram();
...
void LightNode::sendInfoRequest() {
	...
	udpSocket.async_send_to(boost::asio::buffer(infoRequestMsg),
		udpEndpoint,
	...
	switch(p.getID()) {
		case Packet::INFO: {
			...
			if(_pixelCount != pixelCount) {
				...
				std::cout << "[Info] " << name << ": pixel count: " << _pixelCount << std::endl;
			}
...
bool LightNode::update() {
...

	{
		...
		updateMsg = Packet::Update(strip.getPixels()).asDatagram();
	...
	*/
	...
	udpSocket.async_send_to(boost::asio::buffer(updateMsg), udpEndpoint,

...
LightHub::LightHub(uint16_t _sendPort, uint16_t _recvPort, DiscoveryMethod_e _discoveryMethod,
	...
	sendSocket.set_option(boost::asio::ip::udp::socket::reuse_address(true));
	...
	discoveryMsg = Packet::Ping().asDatagram();
...
void LightHub::discover(LightHub::DiscoveryMethod_e method) {
	switch(method) {
		...
		case BROADCAST:
		{
			...
			sendSocket.async_send_to(boost::asio::buffer(discoveryMsg),
